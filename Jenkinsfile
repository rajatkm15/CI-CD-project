                    ...
                    steps {
                        container(name: 'kaniko', shell: '/busybox/sh') {
                            sh '''#!/busybox/sh
                                ...
                                /kaniko/executor --context `pwd`/frontend --dockerfile=Dockerfile --insecure --skip-tls-verify --destination=http://jfrog.192.168.49.2.sslip.io/artifactory/docker-local/frontend:$VERSION-$BUILD_NUMBER --image-name-with-digest-file=frontend-image-file
                            '''
                        }
                    }
                }
                ...
                stage('Backend') {
                    steps {
                        container(name: 'kaniko', shell: '/busybox/sh') {
                            sh '''#!/busybox/sh
                                VERSION=$(grep '"version":' ./backend/package.json | head -1 | awk -F: '{ print $2 }' | sed 's/[", ]//g')
                                /kaniko/executor --context `pwd`/backend --dockerfile=Dockerfile --insecure --skip-tls-verify --destination=http://jfrog.192.168.49.2.sslip.io/artifactory/docker-local/backend:$VERSION-$BUILD_NUMBER --image-name-with-digest-file=backend-image-file
                            '''
                        }
                    }
                }
